...
public class ItemRenderer
{
    ...
    public void renderItemInFirstPerson(float partialTicks)
    {
        ...
        {
            boolean fakeAB = Sleek.getInstance().getModuleManager().getModuleByName("Animations").isToggled() && KillAura.isBlocking && Sleek.getInstance().getModuleManager().getModuleByName("KillAura").isToggled();
            ...
            else if (entityplayersp.getItemInUseCount() > 0 || fakeAB)
            {
                ...
                switch (enumaction)
                {
                    ...
                    case BLOCK:
                        if (Sleek.getInstance().getModuleManager().getModuleByName("Animations").isToggled()) {
                            Animations anim = (Animations) Sleek.getInstance().getModuleManager().getModuleByName("Animations");
