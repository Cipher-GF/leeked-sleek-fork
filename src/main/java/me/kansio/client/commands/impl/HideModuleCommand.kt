...
class HideModuleCommand : Command() {
    override fun run(args: Array<String>) {
        ...
        if (args[0].equals("all", ignoreCase = true)) {
            Client.getInstance().moduleManager.modules.forEach(Consumer { module: Module -> module.isHidden = true })
            ChatUtil.log("You've hid all the modules.")
            return
        }
        val name = args[0]
        Client.getInstance().moduleManager.getModuleByNameIgnoreSpace(name).isHidden = true
