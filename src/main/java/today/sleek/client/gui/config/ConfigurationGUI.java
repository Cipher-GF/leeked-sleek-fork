...
public class ConfigurationGUI extends GuiScreen {
    ...
    public void initGui() {
        ...
        int i = 0;
        for (Config config : Sleek.getInstance().getConfigManager().getConfigs()) {
            i++;
    ...
    public void loadConfigs(int page) {
        ...
        for (int s = i; s < i + 7; s++) {
            try {
                listedConfigs.add(Sleek.getInstance().getConfigManager().getConfigs().get(s));
    ...
    protected void mouseClicked(int mouseX, int mouseY, int mouseButton) throws IOException {
        ...
        if (selectedConfig != null) {
            ...
            if (RenderUtils.hover(sr.getScaledWidth() / 2 + 10, sr.getScaledHeight() / 2 + 88, mouseX, mouseY, 50, 13)) {
                Sleek.getInstance().getConfigManager().loadConfig(selectedConfig.getName(), false);
            ...
            if (RenderUtils.hover(sr.getScaledWidth() / 2 + 80, sr.getScaledHeight() / 2 + 88, mouseX, mouseY, 50, 13)) {
                Sleek.getInstance().getConfigManager().removeConfig(selectedConfig.getName());
        ...
        if (RenderUtils.hover(sr.getScaledWidth() / 2 + 10, sr.getScaledHeight() / 2 - 38, mouseX, mouseY, 50, 13)) {
            ...
            typing = false;
            Sleek.getInstance().getConfigManager().saveConfig(name);
